 ...
 */
package org.esco.portlet.helpinfo.service.bean;
 ...
 */
@Service
public class HelpUrlBuilderImpl implements IHelpUrlBuilder {
    ...
    public String transform(final PortletRequest request, final String url) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("Replacement regexp of helpUrl from {} to {}",url, rewroteUrl );
        }
        ...
        try {
            ...
            if (uri.getScheme() == null && uri.getHost() == null) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("Rewrite helpUrl from {} to {}, with params scheme {}, host {}, port {}, context path {}",rewroteUrl,
                ...
                if (log.isDebugEnabled()) {
                    log.debug("Rewrite helpUrl from {} to {}, with params scheme {}",rewroteUrl,
                            request.getScheme() + ":" + path + rewroteUrl, request.getScheme() + path);
                }
                rewroteUrl = request.getScheme() + ":" + path + rewroteUrl;
            }
        } catch (URISyntaxException e) {
            log.error("The url '" + rewroteUrl + "' to get helpInfos is malformed !", e);
            return null;
        }
        if (log.isDebugEnabled()) {
